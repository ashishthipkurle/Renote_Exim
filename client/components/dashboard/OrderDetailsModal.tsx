        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Order #${order.id.slice(0, 8).toUpperCase()}`} maxWidth="max-w-3xl">
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-8">
                        <div>
                            <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-4 italic">Asset Specification</h4>
                            <div className="flex gap-4 p-4 rounded-2xl bg-muted/20 backdrop-blur-xl border border-border shadow-2xl">
                                ...
                                <div className="flex flex-col justify-center">
                                    <div className="text-sm font-black text-white uppercase italic tracking-tighter leading-tight">{order.product.name}</div>
                        ...
                        <div>
                            <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-4 italic">Origin Node Intelligence</h4>
                    ...
                    <div className="space-y-8">
                        <div>
                            <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-4 italic">Procurement Telemetry</h4>
                            <div className="rounded-2xl border border-border bg-muted/20 backdrop-blur-xl p-5 space-y-4 shadow-2xl">
                                ...
                                <div className="pt-4 border-t border-border flex justify-between items-center">
                                    ...
                                    <span className="text-xl font-black text-white tracking-widest italic">{formatCurrency(order.totalPrice)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {order.status === "SHIPPED" && (
                            <div>
                                <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-4 italic">Logistics Distribution Feed</h4>
                                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between shadow-inner">
                ...
                <div className="pt-8 border-t border-border flex gap-4">
                    <button className="flex-1 bg-white hover:bg-primary/90 text-black font-black uppercase tracking-[0.2em] py-3.5 rounded-2xl shadow-2xl shadow-white/10 transition-all hover:-translate-y-1 active:translate-y-0 text-[10px]">
                        Re-Initialize Procurement
                    </button>
                    <button className="flex-1 bg-muted/40 hover:bg-muted/60 text-white font-black uppercase tracking-[0.2em] py-3.5 rounded-2xl border border-border transition-all hover:-translate-y-1 active:translate-y-0 text-[10px]">
                        Download Asset Log
                    </button>
                </div>
            </div>
