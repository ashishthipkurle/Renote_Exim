...
export default function ShipmentTrackingPanel({ shipment }: { shipment?: any | null }) {
 if (!shipment) {
 return (
 <div className="rounded-lg border border-dashed border-slate-300 dark:border-white/15 bg-slate-50 dark:bg-white/[0.03] p-4">
 ...
 const statusToUse = shipment.currentStatus || shipment.status;
 const currentIndex = statusIndex(statusToUse);
 const pct = currentIndex < 0 ? 0 : Math.max(8, (currentIndex / (SHIPMENT_STEPS.length - 1)) * 100);
 const badgeStyle = SHIPMENT_STATUS_STYLE[statusToUse] ?? "text-slate-600 dark:text-slate-300 bg-slate-500/10 border-slate-500/20";

 return (
 <div className="rounded-lg border border-slate-200 dark:border-white/10 bg-white/70 dark:bg-white/[0.03] p-4 space-y-4 shadow-sm">
 ...
 {String(statusToUse).replaceAll("_", " ")}
 </span>
 </div>
 ...
 <div key={`${i}-${entry?.createdAt ?? entry?.status ?? "line"}`} className="flex items-start gap-2 text-xs">
 ...
 <p className="text-slate-500 dark:text-slate-400">{entry?.location ?? "-"} · {formatDate(entry?.createdAt)}</p>
 </div>
 </div>
 ))}
 </div>
