    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex h-svh w-full bg-board overflow-hidden",
            className
          )}
          {...props}
        >
...
function SidebarInset({ className, style, children, ...props }: React.ComponentProps<"main">) {
  ...
  const noPadding = (props as any)["data-no-padding"] === true

  return (
    <main
      data-slot="sidebar-inset"
      className={cn(
        // remove internal top padding (root layout handles header offset)
        "bg-background relative flex min-w-0 w-full flex-1 flex-col",
        // remove inset margins/rounded corners so border touches sidebar
        className
      )}
      style={mergedStyle}
      {...props}
    >
      {/* Decorative board container behind the page */}
      <div
        aria-hidden={true}
      ...
      <div
        ...
        className={cn(
          ...
          !noPadding && "rounded-2xl shadow-sm bg-background"
        )}
      >
        {children}
      </div>
