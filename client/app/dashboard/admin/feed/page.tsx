    ...
    <div className="h-dvh flex flex-col bg-[#0b1019] relative overflow-hidden">
      ...
      <header className="flex-shrink-0 h-20 px-8 flex items-center justify-between border-b border-white/5 bg-[#0b1019]/30 backdrop-blur-md z-30">
        <div>
          ...
          <p className="text-slate-500 text-xs font-medium italic">Synchronized real-time event stream from across the global infrastructure.</p>
        </div>
          ...
          <div className="space-y-4 relative">
            ...
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-24 bg-white/5 rounded-2xl animate-pulse ml-12" />
              ))
            ) : filteredFeed.length === 0 ? (
              <div className="h-64 flex flex-col items-center justify-center text-slate-500 ml-12 border border-dashed border-white/10 rounded-3xl space-y-4">
