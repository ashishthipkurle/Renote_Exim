        ...
        <div className="space-y-10">
                ...
                <div className="flex flex-wrap gap-3">
                    {statusPills.map((pill) => (
                        <button
                            ...
                            className={`px-6 py-3 rounded-2xl border text-[10px] font-black uppercase tracking-widest italic transition-all backdrop-blur-3xl ${currentStatus === pill.id
            ...
            <div className="space-y-6 relative">
                <AnimatePresence>
                    {isPending && (
                        <motion.div 
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="absolute inset-0 bg-card/40 dark:bg-white/5 backdrop-blur-sm z-50 flex items-center justify-center rounded-[3rem]"
                        >
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <p className="text-[10px] font-black text-foreground dark:text-white uppercase tracking-[0.4em] italic">Streaming Trade Sequence...</p>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
                ...
                {displayOrders.length === 0 ? (
                    <div className="bg-card/40 dark:bg-white/5 backdrop-blur-3xl border border-border dark:border-white/5 shadow-xl dark:shadow-2xl rounded-[3rem] p-24 text-center">
                        <div className="flex flex-col items-center gap-8 opacity-40">
                            ...
                            <div className="space-y-4">
                                <h2 className="text-2xl font-black text-foreground dark:text-white uppercase italic tracking-tighter">Null_Trade_Registry</h2>
                    ...
                    <>
                        {/* Desktop Header */}
                        <div className="hidden lg:grid grid-cols-12 gap-10 px-10 text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.4em] italic">
                        ...
                        <div className="space-y-4">
                            {displayOrders.map((order) => {
                                ...
                                return (
                                    <div
                                        key={order.id}
                                        className={`bg-card/40 dark:bg-white/5 backdrop-blur-3xl border border-border dark:border-white/5 hover:border-border dark:border-white/20 transition-all duration-700 shadow-xl dark:shadow-2xl rounded-[2.5rem] p-6 lg:p-8 group ${isExpanded ? "ring-2 ring-white/10" : "hover:-translate-y-1"}`}
                                    >
                                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
                                            ...
                                            >
                                                <div className="size-20 rounded-[2rem] bg-black/5 dark:bg-white/10 border border-border dark:border-white/5 flex-shrink-0 flex items-center justify-center overflow-hidden p-2 group-hover:border-border dark:border-white/20 transition-all duration-1000 shadow-inner">
                                                ...
                                                <div className="min-w-0">
                                                    <div className="text-xl font-black text-foreground dark:text-white truncate italic tracking-tighter uppercase group-hover:translate-x-1 transition-transform">{order.product?.name ?? "NULL_ASSET"}</div>
                                                    <div className="text-[10px] text-muted-foreground/30 mt-2 font-black uppercase tracking-widest italic flex items-center gap-2 group-hover:text-muted-foreground transition-colors truncate">
                                                        {order.importer?.companyName || order.importer?.name || "ANON_NODE"} // {order.importer?.country ?? "GLOBAL"}
                                                    </div>
                                                    <div className="text-[9px] text-muted-foreground/10 mt-1 font-black uppercase tracking-widest italic">
                                            ...
                                            <div className="lg:col-span-2 flex justify-center">
                                                <span className={`inline-flex items-center gap-3 px-6 py-2.5 rounded-full border text-[9px] font-black uppercase tracking-[0.2em] italic shadow-xl dark:shadow-2xl transition-all ${cfg.color}`}>
                                            ...
                                            <div className="lg:col-span-1 text-center">
                                                <div className="text-[10px] text-foreground dark:text-white font-black italic tracking-widest uppercase opacity-60">{formatDate(order.createdAt)}</div>
                                            </div>
                                                        ...
                                                        <div className="pt-10 mt-10 border-t border-border dark:border-white/5 space-y-10">
                                                            <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
                                                                <div className="xl:col-span-3">
                                                                    <div className="aspect-[4/3] rounded-[2rem] border border-border dark:border-white/5 bg-white/[0.02] overflow-hidden p-3 shadow-inner group/img">
                                                                        {order.product?.images?.[0] ? (
                                                                            <img src={order.product.images[0]} alt="" className="w-full h-full object-cover rounded-2xl grayscale group-hover/img:grayscale-0 transition-all duration-1000 opacity-60 group-hover/img:opacity-100" />
                                                                        ) : (
                                                                ...
                                                                <div className="xl:col-span-9 grid grid-cols-1 md:grid-cols-3 gap-6">
                                                                    {[
                                                                        { l: 'Buyer Entity', v: order.importer?.companyName || order.importer?.name || "ANON_NODE", i: Globe },
                                                                        { l: 'Node Origin', v: order.importer?.country ?? "GLOBAL", i: ShieldCheck },
                                                                        { l: 'Protocol Value', v: formatCurrency(order.totalPrice), i: TrendingUp },
                                                                        { l: 'Signal Quantity', v: order.quantity ?? 0, i: Layers },
                                                                        { l: 'Reference Key', v: order.orderNumber || order.id?.slice(0,8).toUpperCase(), i: Package },
                                                                        { l: 'Settlement Status', v: (order.paymentStatus || "PENDING").toUpperCase(), i: Clock },
                                                                    ].map(detail => (
                                                                        <div key={detail.l} className="p-6 rounded-[1.5rem] bg-white/[0.02] border border-border dark:border-white/5 group/detail hover:border-border dark:border-white/20 transition-all">
                                                                            <div className="flex items-center gap-4 mb-3">
                                                                                <detail.i className="w-4 h-4 text-muted-foreground/20 group-hover/detail:text-foreground dark:text-white transition-colors" />
                                                                                <span className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-widest italic">{detail.l}</span>
                                                                            </div>
                                                                            <div className="text-foreground dark:text-white font-black text-sm italic uppercase tracking-tighter truncate">{detail.v}</div>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="p-8 rounded-[2rem] border border-border dark:border-white/10 bg-black/5 dark:bg-white/10 relative overflow-hidden group/intel">
                                                                ...
                                                                <h4 className="text-[10px] font-black text-foreground dark:text-white uppercase tracking-[0.3em] mb-6 italic flex items-center gap-3">
                                                                ...
                                                                {review ? (
                                                                    <div className="space-y-4">
                                                                        <div className="flex items-center gap-2 text-foreground dark:text-white">
                                                                            ...
                                                                            <span className="text-[10px] font-black text-white/40 ml-4 tracking-[0.2em] italic uppercase">{review.rating} / 5.0 CONFIRMED</span>
                                                                        </div>
                                                                        <p className="text-sm font-black text-white/60 leading-relaxed italic uppercase tracking-tighter">&ldquo;{review.comment || "Consensus confirmed. No anomalies reported in trade sequence."}&rdquo;</p>
                                                                    </div>
                                                                ) : (
                                                                    <p className="text-[10px] font-black text-muted-foreground/20 uppercase tracking-[0.4em] italic mb-2">Awaiting procurement node validation sequence...</p>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </motion.div>
