...
export default async function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const cfg = STATUS_CONFIG[order?.orderStatus || "PENDING"] ?? STATUS_CONFIG.PENDING;
  const StatusIcon = cfg.icon;

  const productImages = Array.isArray(orderWithRel?.product?.images) 
    ? orderWithRel.product.images 
    : [];
      ...
      <div className="flex-shrink-0 p-6 lg:p-8 border-b border-border bg-header/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="bg-card border border-border p-4 rounded-lg flex items-center gap-6">
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-8 custom-scrollbar">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-gradient-to-br from-white/5 to-transparent border border-border dark:border-white/10 p-6 rounded-lg shadow-2xl shadow-white/5">
              ...
              <OrderActions
                ...
                importerCountry={orderWithRel?.buyer?.country || ""}
              />
            </section>
            ...
            <div className="bg-card border border-border rounded-lg overflow-hidden">
              ...
              <div className="p-6">
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="w-full md:w-48 h-48 rounded-lg bg-muted border border-border flex items-center justify-center overflow-hidden flex-shrink-0 shadow-inner">
                    {productImages.length > 0 ? (
                      <img src={productImages[0]} alt={orderWithRel?.product?.name || "Product"} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      <h2 className="text-2xl font-bold text-foreground mb-1">{orderWithRel?.product?.name || "Unknown Product"}</h2>
                      <div className="flex gap-2">
                        <span className="text-[10px] bg-muted text-muted-foreground px-2 py-0.5 rounded border border-border">
                          {orderWithRel?.product?.category || "Unknown"}
                        </span>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                      <div className="bg-muted/30 p-3 rounded-xl border border-border">
                        ...
                        <div className="text-lg font-bold">{order?.quantity || 0} units</div>
                      </div>
                      <div className="bg-muted/30 p-3 rounded-xl border border-border">
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-foreground leading-tight">
                      {orderWithRel?.buyer?.businessName || orderWithRel?.buyer?.name || "Unknown"}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-foreground capitalize">
                      {orderWithRel?.buyer?.country || "International Merchant"}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-foreground">{orderWithRel?.buyer?.name || "Unknown"}</div>
                    <div className="text-[10px] text-muted-foreground mt-0.5 font-mono">{orderWithRel?.buyer?.email || ""}</div>
                  </div>
                </div>
                ...
                <div className="mt-6">
                  <OrderContact 
                    ...
                    receiverName={orderWithRel?.buyer?.name || "Buyer"}
                  />
                </div>
              </div>
            </div>

            <div className="bg-muted/50 dark:bg-white/5 border border-border rounded-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4 opacity-50">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground/40 mt-0.5 ">Awaiting action...</div>
                  </div>
                </div>
              </div>
            </div>
