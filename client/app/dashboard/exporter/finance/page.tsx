...
function FinancialChart({ data, dark }: { data: MonthlyPoint[]; dark: boolean }) {
  ...
  if (!data.length) return (
    <div className="h-[250px] flex items-center justify-center opacity-20 italic text-[10px] uppercase font-black tracking-[0.3em] text-foreground dark:text-white">
    ...
    <div className="relative w-full group/chart">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto block overflow-visible">
        ...
        <g transform={`translate(${padL},${padT})`}>
          ...
          {pts.map((p, i) => (
            <g key={i}>
              <text x={p.x} y={cH + 30} textAnchor="middle" fill="#ffffff" fillOpacity="0.2" fontSize="9" fontWeight="black" className="uppercase tracking-widest italic">
      ...
      {tooltip && (
        ...
        >
          ...
          <div className="text-xl font-black italic tracking-tighter uppercase whitespace-nowrap">
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-card dark:bg-[#0a0a0a]">
      <div className="flex flex-col items-center gap-6 opacity-40">
        ...
        <p className="text-[10px] font-black text-foreground dark:text-white uppercase tracking-[0.4em] italic">Indexing Capital Nodes...</p>
      </div>
    </div>
  );
    ...
    <div className="h-full overflow-hidden flex flex-col bg-background selection:bg-primary selection:text-primary-foreground">
      ...
      <header className="flex-shrink-0 px-10 py-10 border-b border-border dark:border-white/5 bg-background/40 backdrop-blur-3xl z-40">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-10">
          <div>
            <h1 className="text-5xl font-black tracking-tighter text-foreground dark:text-white uppercase italic">Capital Intel</h1>
          ...
          <button
            ...
            className="inline-flex items-center gap-3 bg-primary hover:bg-primary/90 text-primary-foreground font-black text-[10px] uppercase tracking-[0.2em] italic py-4 px-10 rounded-2xl shadow-2xl shadow-white/5 transition-all active:scale-95 group"
          >
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-16 custom-scrollbar animate-in fade-in slide-in-from-bottom-4 duration-1000">
          ...
          {[
            ...
            <div key={s.k} className="bg-card/40 dark:bg-white/5 backdrop-blur-3xl border border-border dark:border-white/5 p-10 relative overflow-hidden group rounded-[2.5rem] transition-all hover:border-border dark:border-white/10 shadow-xl dark:shadow-2xl">
              ...
              <div className="flex items-start justify-between relative z-10 mb-8">
                ...
                <div className="p-4 rounded-2xl bg-black/5 dark:bg-white/10 border border-border dark:border-white/10 text-foreground dark:text-white transition-all group-hover:scale-110">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 max-w-[1700px] mx-auto items-start">
          ...
          <div className="xl:col-span-4 space-y-8">
            <div className="p-10 bg-card/40 dark:bg-white/5 border border-border dark:border-white/5 rounded-[3rem] shadow-xl dark:shadow-2xl backdrop-blur-3xl space-y-10">
              <h3 className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.3em] italic">Yield_Breakdown</h3>
              ...
              <button className="w-full h-16 bg-primary hover:bg-primary/90 text-primary-foreground font-black rounded-2xl text-[10px] uppercase tracking-[0.3em] italic shadow-xl dark:shadow-2xl transition-all active:scale-95">
            ...
            <div className="p-8 rounded-[2rem] bg-black/5 dark:bg-white/10 border border-border dark:border-white/10 flex items-center justify-between group cursor-default">
              <div className="flex items-center gap-5">
                ...
                <p className="text-[9px] font-black text-foreground dark:text-white uppercase tracking-[0.2em] italic">Encryption_AES_256</p>
              </div>
          ...
          {filteredInvoices.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredInvoices.map((inv) => {
                ...
                return (
                  <div key={inv.id} className="bg-card/40 dark:bg-white/5 backdrop-blur-3xl border border-border dark:border-white/5 hover:border-border dark:border-white/20 transition-all duration-700 shadow-xl dark:shadow-2xl rounded-[2.5rem] p-8 flex flex-col xl:flex-row gap-10 items-center group hover:-translate-y-1">
                    <div className="flex items-center gap-8 flex-1 min-w-0">
                      <div className="size-16 rounded-2xl bg-black/5 dark:bg-white/10 border border-border dark:border-white/5 flex items-center justify-center text-muted-foreground group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-700">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[9px] text-muted-foreground/20 mt-2 font-black uppercase tracking-widest italic group-hover:text-muted-foreground/60 transition-colors">
                    ...
                    <div className="grid grid-cols-2 xl:flex xl:items-center gap-12 w-full xl:w-auto">
                      <div className="text-left xl:text-right">
                        <div className="text-muted-foreground/10 text-[8px] uppercase tracking-[0.3em] font-black italic mb-2">Timestamp</div>
                        <div className="text-foreground dark:text-white font-black text-[10px] italic tracking-widest uppercase">{fmtDate(inv.paidAt)}</div>
                      </div>
                      <div className="text-left xl:text-right">
                        <div className="text-muted-foreground/10 text-[8px] uppercase tracking-[0.3em] font-black italic mb-2">Total_Value</div>
                        <div className="text-foreground dark:text-white font-black text-2xl italic tracking-tighter">{fmtUSD(inv.amount)}</div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className={`px-6 py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest italic flex items-center gap-3 border shadow-xl dark:shadow-2xl transition-all ${cfg.color}`}>
