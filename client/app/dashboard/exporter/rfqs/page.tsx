...
export default function ExporterRFQs() {
 ...
 <div>
 <h1 className="text-5xl font-black tracking-tighter text-foreground dark:text-white uppercase ">Requirement Feed</h1>
 ...
 className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === cat ? 'bg-primary text-primary-foreground shadow-xl dark:shadow-2xl' : 'text-muted-foreground/40 hover:text-foreground dark:text-white'}`}
 >
 {cat}
 </button>
 ))}
 ...
 {[1, 2, 3, 4, 5, 6].map(i => (
 <div key={i} className="h-96 bg-card/40 dark:bg-white/5 rounded-lg animate-pulse border border-border dark:border-white/5" />
 ))}
 </div>
 ...
 <h2 className="text-2xl font-black text-foreground dark:text-white uppercase tracking-tighter">Null_Requirement_Index</h2>
 ...
 <div key={rfq.id} className="group flex flex-col bg-card/40 dark:bg-white/5 backdrop-blur-3xl border border-border dark:border-white/5 hover:border-border dark:border-white/20 transition-all duration-700 shadow-xl dark:shadow-2xl rounded-lg overflow-hidden hover:-translate-y-2">
 ...
 <h3 className="text-2xl font-black text-foreground dark:text-white mb-4 tracking-tighter uppercase group-hover:translate-x-1 transition-transform">
 {rfq.title}
 </h3>

 <p className="text-muted-foreground/40 text-xs font-medium line-clamp-3 mb-10 leading-relaxed group-hover:text-muted-foreground/80 transition-colors uppercase tracking-tight">
 {rfq.description}
 </p>

 <div className="grid grid-cols-2 gap-8 pt-8 border-t border-border dark:border-white/5 relative z-10">
 <div>
 <p className="text-[9px] font-black text-muted-foreground/10 uppercase tracking-[0.3em] mb-3 ">Asset_Quantity</p>
 <p className="text-foreground dark:text-white font-black text-lg tracking-widest uppercase">{rfq.quantity} {rfq.unit}</p>
 </div>
 <div>
 <p className="text-[9px] font-black text-muted-foreground/10 uppercase tracking-[0.3em] mb-3 ">Max_Yield_USD</p>
 <p className="text-foreground dark:text-white font-black text-lg tracking-widest uppercase">{rfq.budget ? `$${rfq.budget.toLocaleString()}` : 'NEGOTIABLE'}</p>
 </div>
 </div>
 </div>
 ...
 <div className="flex items-center gap-3 text-[9px] font-black text-muted-foreground/20 uppercase tracking-widest group-hover:text-foreground dark:text-white transition-colors">
 <MessageSquare className="w-4 h-4" />
 {rfq._count?.quotes || 0} Telemetry Quotes Sent
 </div>
 <div className="flex items-center gap-3 text-foreground dark:text-white font-black text-[9px] uppercase tracking-widest bg-black/5 dark:bg-white/10 px-4 py-2 rounded-full">
 ...
 className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-black text-[10px] uppercase tracking-[0.3em] h-14 rounded-lg group/btn shadow-xl dark:shadow-2xl flex items-center justify-center gap-3 transition-all active:scale-95 "
 >
 ...
 <h3 className="text-foreground dark:text-white font-black text-2xl uppercase tracking-tighter mb-4">Direct Response Protocol</h3>
