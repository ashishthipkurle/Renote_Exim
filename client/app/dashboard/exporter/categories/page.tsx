        ...
        <div className="max-w-[1200px] mx-auto">
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-500">
              ...
              <p className="text-sm">No categories yet — list some products to see them here.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div key={c.category} className={`bg-gradient-to-br ${colors} backdrop-blur-xl border shadow-xl rounded-2xl p-6 hover:scale-[1.02] transition-transform`}>
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-emerald-400 font-black text-xl">{formatCurrency(c.revenue)}</div>
                      </div>
                    </div>
                    <div className="mt-4 h-1 w-full bg-slate-800/60 rounded-full overflow-hidden">
                      <div className="h-full bg-white/20 rounded-full transition-all" style={{ width: `${Math.max((c.revenue / maxRev) * 100, 5)}%` }} />
                    </div>
                  </div>
                );
              })}
