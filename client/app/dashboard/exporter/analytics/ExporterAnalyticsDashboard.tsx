...
function ChartTooltip({ active, payload, label }: any) {
 ...
 return (
 <div className="bg-black/90 border border-border dark:border-white/10 rounded-lg px-5 py-4 shadow-xl dark:shadow-2xl backdrop-blur-xl">
 <p className="text-[10px] text-muted-foreground mb-3 font-black uppercase tracking-widest ">{label}</p>
 ...
 <span style={{ color: p.color }} className="">
...
function EmptyChart({ icon: Icon, message }: { icon: LucideIcon; message: string }) {
 ...
 <div className="p-5 rounded-lg bg-black/5 dark:bg-white/10 border border-border dark:border-white/10">
 <Icon className="w-10 h-10 text-foreground dark:text-white" />
 </div>
 <p className="text-foreground dark:text-white text-[10px] font-black uppercase tracking-[0.2em] text-center max-w-[250px] ">{message}</p>
 </div>
 );
}
...
function KpiCard({
 ...
 return (
 <div
 className="relative overflow-hidden rounded-lg border border-border dark:border-white/5 bg-card/40 dark:bg-white/5 p-8 transition-all duration-500 hover:-translate-y-1 group cursor-default shadow-xl dark:shadow-2xl backdrop-blur-3xl"
 >
 <div
 ...
 <p className="text-4xl font-black tracking-tighter text-foreground dark:text-white uppercase group-hover:scale-[1.02] transition-transform duration-300 origin-left">
 {value}
 </p>
 <p className="text-[9px] font-black text-muted-foreground/40 mt-2 uppercase tracking-widest">{sub}</p>
 {trend !== undefined && (
 <div
 className={`mt-6 inline-flex items-center gap-2 text-[9px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-all ${isPositive
...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
 return (
 <div
 className={`rounded-lg border border-border dark:border-white/5 bg-card/40 dark:bg-white/5 p-8 shadow-xl dark:shadow-2xl backdrop-blur-3xl transition-all duration-300 hover:border-border dark:border-white/10 ${className}`}
 >
 {children}
 </div>
 );
...
function CardHeader({ title, subtitle }: { title: string; subtitle?: string }) {
 return (
 <div className="mb-8">
 <p className="text-xl font-black text-foreground dark:text-white uppercase tracking-tighter">{title}</p>
 {subtitle && <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-1 opacity-40 ">{subtitle}</p>}
 </div>
 );
}
 ...
 * { margin: 0; padding: 0; box-sizing: border-box; }
 ...
 <div>
 <h1 className="text-4xl font-black tracking-tighter text-foreground dark:text-white uppercase ">Intelligence Analytics</h1>
 ...
 {/* Timeframe */}
 <div className="flex items-center bg-black/5 dark:bg-white/10 border border-border dark:border-white/10 rounded-lg p-1.5 backdrop-blur-md">
 {TIMEFRAMES.map((tf) => (
 <button
 key={tf}
 ...
 className="pl-12 pr-10 py-3 bg-black/5 dark:bg-white/10 border border-border dark:border-white/10 rounded-lg text-[10px] text-foreground dark:text-white font-black uppercase tracking-widest focus:outline-none focus:border-white/40 appearance-none cursor-pointer hover:bg-black/10 dark:bg-white/15 transition-colors"
 >
 ...
 className="inline-flex items-center gap-3 bg-primary hover:bg-primary/90 active:scale-95 text-primary-foreground font-black py-3 px-6 rounded-lg shadow-2xl shadow-white/5 transition-all text-[10px] uppercase tracking-[0.2em] "
 >
 ...
 <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-1 opacity-40 ">
 ...
 <div className="flex bg-black/5 dark:bg-white/10 border border-border dark:border-white/10 rounded-lg p-1.5 backdrop-blur-md">
 ...
 return (
 <div className="bg-black/90 border border-border dark:border-white/10 rounded-lg px-5 py-4 shadow-xl dark:shadow-2xl backdrop-blur-xl">
 <p className="text-[10px] text-muted-foreground mb-2 font-black uppercase tracking-widest ">
 {p.name ? (CATEGORY_LABELS[p.name as string] ?? p.name) : "NULL_ENTITY"}
 </p>
 <p className="text-sm font-black text-foreground dark:text-white ">{fCurrency(p.value as number)}</p>
 ...
 <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex-1 group-hover:text-foreground dark:text-white transition-colors ">{CATEGORY_LABELS[c.category] ?? c.category}</span>
 <span className="text-[9px] text-muted-foreground/20 font-black uppercase tracking-tighter">[{c.orderCount} SIG]</span>
 <span className="text-[10px] font-black text-foreground dark:text-white w-12 text-right ">{pct}%</span>
 </div>
 );
 })}
 </div>
 ...
 <div>
 <p className="text-sm text-foreground dark:text-white font-black uppercase tracking-tighter group-hover:translate-x-1 transition-transform">{p.name}</p>
 ...
 <p className="text-lg font-black uppercase" style={{ color }}>{fCurrency(p.revenue)}</p>
 </div>
 ...
 <div>
 <p className="text-xl font-black text-foreground dark:text-white uppercase tracking-tighter">Geographic Distribution</p>
 <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-1 opacity-40 ">Global Grid Coverage: {data.geographicData.length} active nodes</p>
 </div>
 </div>
 ...
 <div
 key={g.country}
 className="flex items-center gap-4 p-5 rounded-lg bg-white/[0.02] border border-border dark:border-white/5 hover:border-border dark:border-white/10 hover:bg-white/[0.05] transition-all group"
 >
 <div
 ...
 <p className="text-xs font-black text-foreground dark:text-white uppercase tracking-wider truncate">{g.country}</p>
 ...
 <p className="text-sm font-black uppercase" style={{ color: PALETTE[i % PALETTE.length] }}>{fCurrency(g.revenue)}</p>
 ...
 <div>
 <p className="text-xl font-black text-foreground dark:text-white uppercase tracking-tighter">Capital Settlement</p>
 <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-1 opacity-40 ">Signal Liquidity Status</p>
 </div>
 </div>
 ...
 return (
 <div className="bg-black/90 border border-border dark:border-white/10 rounded-lg px-5 py-4 shadow-xl dark:shadow-2xl backdrop-blur-xl">
 <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-1" style={{ color: p.payload.color }}>{p.name}</p>
 <p className="text-sm font-black text-foreground dark:text-white ">{fNum(p.value as number)} transmissions</p>
 </div>
 );
 }} />
 </PieChart>
 ...
 <p className="text-4xl font-black text-foreground dark:text-white ">{data.paymentBreakdown.PAID}</p>
 ...
 <div
 key={s.label}
 className="flex items-center gap-4 p-4 rounded-lg border transition-all hover:bg-white/[0.02]"
 ...
 <p className="text-lg font-black text-foreground dark:text-white ">{fNum(s.count)}</p>
