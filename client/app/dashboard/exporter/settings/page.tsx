    ...
    <div className="h-full overflow-hidden flex flex-col bg-gradient-to-br from-[#0a0c12] via-[#0d1017] to-[#0a0c12]">
      <header className="flex-shrink-0 p-6 lg:p-8 border-b border-white/5">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage your profile and company information.</p>
        </div>
      </header>
      <div className="flex-1 overflow-y-auto p-6 lg:p-8 custom-scrollbar">
        <div className="max-w-[800px] mx-auto space-y-8">
          {loading ? (
            ...
            <>
              {/* Avatar + Email header */}
              <div className="bg-[#151c2a]/60 backdrop-blur-xl border border-white/5 rounded-2xl p-6 flex items-center gap-6">
                ...
                <div>
                  <div className="text-white font-black text-xl tracking-tight uppercase italic">{profile?.name || "—"}</div>
                  ...
                  {profile?.companyName && (
                    <div className="text-xs text-slate-500 mt-1 font-medium">{profile.companyName}</div>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Basic Info */}
                <div className="bg-[#151c2a]/60 backdrop-blur-xl border border-white/5 rounded-3xl p-8 space-y-6">
                  <h3 className="text-sm font-black text-white tracking-[0.2em] uppercase opacity-50 mb-4 italic">Basic Information</h3>
                  {fields.slice(0, 5).map((f) => (
                    <div key={f.key}>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2.5 ml-1">{f.label}</label>
                      <input
                        type={f.type || "text"}
                        value={form[f.key]}
                        onChange={(e) => setForm((prev) => ({ ...prev, [f.key]: e.target.value }))}
                        placeholder={f.placeholder}
                        className="w-full bg-slate-900/60 border border-white/5 focus:border-primary/50 rounded-2xl px-5 py-3.5 text-sm text-white placeholder:text-slate-600 focus:outline-none transition-all shadow-inner"
                      />
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-8">
                  <div className="bg-[#151c2a]/60 backdrop-blur-xl border border-white/5 rounded-3xl p-8 space-y-6">
                    <h3 className="text-sm font-black text-white tracking-[0.2em] uppercase opacity-50 mb-4 italic">Social & Online</h3>
              ...
              {/* About Company */}
              <div className="bg-[#151c2a]/60 backdrop-blur-xl border border-white/5 rounded-3xl p-8 space-y-6">
                <h3 className="text-sm font-black text-white tracking-[0.2em] uppercase opacity-50 mb-4 italic">About Your Company</h3>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2.5 ml-1 text-right">Company Description</label>
                  <textarea
                    value={form.description}
                    onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))}
                    placeholder="Tell importers about your expertise, production capacity, and values..."
                    rows={6}
                    className="w-full bg-slate-900/60 border border-white/5 focus:border-primary/50 rounded-2xl px-5 py-4 text-sm text-white placeholder:text-slate-600 focus:outline-none transition-all shadow-inner resize-none"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col items-center gap-4 py-8">
                ...
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="group relative px-12 py-4 bg-primary hover:bg-[#0f49bd] disabled:opacity-50 text-white font-black text-xs uppercase tracking-[0.3em] rounded-2xl shadow-2xl shadow-primary/30 transition-all active:scale-95 overflow-hidden"
                >
