...
export default function SupplierOrdersPage() {
 ...
 className="w-full bg-card/30 border border-white/5 rounded-lg pl-14 pr-6 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/20 transition-all font-bold placeholder:uppercase placeholder:text-[10px] placeholder:tracking-widest"
 value={search}
 onChange={(e) => setSearch(e.target.value)}
 />
 </div>
 <Button variant="outline" className="h-14 px-6 rounded-lg border-white/5 bg-card/30 font-bold uppercase text-[10px] tracking-widest gap-3">
 ...
 className="group"
 >
 <Card className="bg-card/20 border-white/5 rounded-lg overflow-hidden hover:border-[#D4AF37]/30 transition-all duration-500 shadow-xl">
 <CardContent className="p-8">
 <div className="flex flex-col lg:flex-row lg:items-center gap-8">
 {/* Order Info */}
 <div className="flex items-center gap-6 lg:w-[25%] shrink-0">
 <div className="size-16 rounded-lg bg-muted/20 flex items-center justify-center text-[#D4AF37]">
 <Package className="w-8 h-8 opacity-40 group-hover:opacity-100 transition-opacity" />
 </div>
 <div>
 <h3 className="font-black uppercase tracking-tighter text-lg">{order.orderNumber}</h3>
 <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-40 ">{order.date}</p>
 </div>
 </div>
 ...
 <p className="text-[8px] font-black uppercase tracking-widest text-[#D4AF37]/60 ">Product Specification</p>
 <p className="font-bold text-sm uppercase tracking-tight">{order.productName}</p>
 </div>
 <div className="space-y-1">
 <p className="text-[8px] font-black uppercase tracking-widest text-primary/60 ">Destination Node</p>
 ...
 <p className="font-black tracking-tighter tabular-nums">{order.quantity}</p>
 </div>
 <div className="text-right">
 <p className="text-[8px] font-black tracking-widest uppercase opacity-40 mb-1">Value</p>
 <p className="font-black tracking-tighter tabular-nums text-emerald-500">{formatCurrency(order.totalValue)}</p>
 </div>
 </div>
