...
import { FileText, Plus, Search, Filter, Clock, MessageSquare, AlertCircle, Package } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Your Custom RFQs</h1>
          <p className="text-slate-400 mt-1">Manage your active sourcing requirements and exporter quotes.</p>
        </div>
        ...
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          ...
          <DialogContent className="bg-slate-950 border-slate-900 max-w-2xl text-white">
            <DialogHeader>
        ...
        <div className="md:col-span-3">
          {loading ? (
            ...
            <div className="text-center py-20 bg-slate-900/30 border-2 border-dashed border-slate-800 rounded-3xl">
              ...
              <h3 className="text-xl font-bold text-white">No active RFQs</h3>
              <p className="text-slate-400 max-w-xs mx-auto mt-2 mb-8">
                Request custom quotes from verified international exporters.
              </p>
              <Button onClick={() => setIsFormOpen(true)} variant="outline" className="border-slate-800 text-slate-300">
                Create First RFQ
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              {rfqs.map((rfq) => (
                <Link 
                  ...
                  className="group p-6 bg-slate-900/50 border border-slate-800 rounded-2xl hover:border-blue-500/30 transition-all block"
                >
                  ...
                  <div className="flex flex-wrap items-center gap-6 pt-6 border-t border-slate-800/50">
                    ...
                    <div className="flex items-center gap-2 text-xs font-bold text-blue-500 ml-auto">
          ...
          <div className="p-6 border border-slate-800 rounded-2xl bg-slate-900/50">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-300">Open Requests</span>
                <span className="text-sm font-bold text-white">{rfqs.filter(r => r.status === 'OPEN').length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-300">Total Quotes</span>
