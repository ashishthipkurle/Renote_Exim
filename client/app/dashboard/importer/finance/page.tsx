...
export default function ImporterFinancePage() {
 ...
 className="bg-muted hover:bg-muted/30 text-foreground border border-border font-black text-[10px] uppercase tracking-[0.2em] py-3.5 px-8 rounded-lg transition-all flex items-center gap-3 active:scale-95"
 >
 ...
 <div className="bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-6 relative overflow-hidden group">
 ...
 <div className="text-3xl font-black text-foreground tracking-tighter">{loading ? "..." : formatCurrency(data?.totalBalance ?? 0)}</div>
 ...
 <div className="text-3xl font-black text-foreground tracking-tighter">{loading ? "..." : formatCurrency(data?.pendingPayouts ?? 0)}</div>
 ...
 <div className="text-3xl font-black text-foreground tracking-tighter">{loading ? "..." : formatCurrency(data?.estTaxLiability ?? 0)}</div>
 <div className="mt-4 flex items-center gap-2 text-muted-foreground text-[10px] font-black uppercase tracking-widest leading-none">
 ...
 <div className="text-3xl font-black text-foreground tracking-tighter">{loading ? "..." : formatCurrency(data?.monthlyBudget ?? 0)}</div>
 ...
 {/* Spending Chart */}
 <section className="lg:col-span-8 bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-8">
 <div className="flex items-center justify-between mb-10">
 <div>
 <h3 className="text-xl font-black text-foreground flex items-center gap-3 uppercase ">
 Spend Trajectory
 ...
 {/* Invoices List */}
 <section className="lg:col-span-4 bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-8 flex flex-col">
 <div className="flex items-center justify-between mb-8">
 <h3 className="text-xl font-black text-foreground uppercase ">Ledger</h3>
 ...
 <div key={inv.id} className="group p-5 bg-muted/20 border border-border hover:border-border rounded-lg transition-all shadow-xl">
 ...
 <div>
 <div className="text-[10px] font-black text-foreground font-mono uppercase truncate max-w-[120px] tracking-tighter">{inv.id}</div>
 ...
 <h3 className="text-xl font-black text-foreground uppercase tracking-tighter shadow-[0_0_20px_rgba(255,255,255,0.1)]">Resource Node Integration</h3>
 ...
 className="w-full bg-muted border border-border rounded-lg py-5 pl-12 pr-6 text-foreground font-black text-xl focus:ring-1 focus:ring-white/20 outline-none transition-all shadow-2xl"
 placeholder="0.00"
 />
 </div>
 </div>
 ...
 className="w-full bg-primary hover:bg-primary/90 text-primary-foreground border-transparent font-black py-4 rounded-lg shadow-xl shadow-primary/5 transition-all active:scale-95 border border-border"
 >
