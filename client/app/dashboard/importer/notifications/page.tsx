          ...
          <div className="flex gap-3">
            <div className="flex bg-slate-800/60 rounded-lg overflow-hidden border border-white/5">
              {(["all", "unread"] as const).map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 text-xs font-bold uppercase tracking-wider transition-colors ${filter === f ? "bg-[#00f0ff] text-black" : "text-slate-400 hover:text-white"}`}>
                  {f}
                </button>
              ))}
            </div>
            <button onClick={markAllRead} className="bg-[#00f0ff] hover:bg-[#00d4e0] text-black font-bold py-2 px-5 rounded-xl text-xs shadow-lg shadow-[#00f0ff]/20 transition-colors">
              Mark All Read
            </button>
          </div>
        </div>
        ...
        <div className="max-w-[900px] mx-auto space-y-3">
          {loading ? (
            ...
            data.notifications.map((n) => (
              <div key={n.id} onClick={() => !n.read && markOneRead(n.id)} className={`bg-[#151c2a]/60 backdrop-blur-xl border rounded-2xl p-5 flex gap-4 transition-all cursor-pointer hover:bg-[#1a2236]/80 ${n.read ? "border-white/5 opacity-60" : "border-[#00f0ff]/20 shadow-lg shadow-[#00f0ff]/5"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!n.read && <span className="w-2 h-2 bg-[#00f0ff] rounded-full flex-shrink-0" />}
                  </div>
