        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className={`px-6 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border ${showUnreadOnly
            ...
            <button
              ...
              className="px-6 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest bg-muted/40 hover:bg-muted/60 text-foreground border border-border transition-all flex items-center gap-3 shadow-xl active:scale-95"
            >
          ...
          {NOTIF_TYPES.map((t) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-5 rounded-lg transition-all group scale-95 hover:scale-100 ${activeType === t.id
          ...
          <div className="max-w-[1000px] mx-auto space-y-4">
            {loading && !data ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-28 bg-muted/20 rounded-lg animate-pulse border border-border/50" />
              ))
              ...
              <div className="py-32 text-center opacity-40 select-none">
                ...
                <h2 className="text-xl font-black text-foreground uppercase tracking-widest ">Signal is Clear</h2>
              ...
              data.notifications.map((n) => (
                <div
                  ...
                  className={`group relative bg-muted/40 backdrop-blur-xl border transition-all rounded-lg p-7 lg:p-10 flex items-start gap-8 cursor-pointer shadow-2xl ${n.read
                    ? "border-border opacity-40 hover:opacity-80"
                    : "border-border shadow-[0_0_30px_rgba(255,255,255,0.05)] hover:border-white/30"
                    }`}
                >
                  <div className={`flex-shrink-0 size-14 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110 ${getTypeColor(n.type)}`}>
                    {getIcon(n.type)}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-4">
                        <h3 className={`text-xl font-black tracking-tight uppercase truncate ${n.read ? "text-muted-foreground" : "text-foreground"}`}>
                          {n.title}
                        </h3>
                        {!n.read && (
