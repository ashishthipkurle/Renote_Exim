...
export default function ImporterAnalyticsPage() {
 ...
 <div>
 <h1 className="text-3xl font-black tracking-tight text-foreground uppercase ">Trade Intelligence Node</h1>
 ...
 {kpis.map((s) => (
 <div key={s.k} className="bg-muted/40 backdrop-blur-xl border border-border shadow-xl p-6 rounded-lg group transition-all hover:bg-muted/60">
 <div className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{s.k}</div>
 <div className="mt-3 text-2xl font-black text-foreground tracking-tighter ">{loading ? "..." : s.v}</div>
 ...
 <div className="bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-6">
 <div className="flex items-center justify-between mb-8">
 <div>
 <div className="text-foreground font-black uppercase tracking-widest leading-none">Procurement Trends</div>
 <div className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1">Monthly breakdown of trading volume</div>
 </div>
 </div>
 {loading ? (
 <div className="h-56 rounded-lg bg-muted/20 border border-border animate-pulse shadow-inner" />
 ) : (data?.monthlySpend?.length ?? 0) === 0 ? (
 <div className="h-56 rounded-lg bg-muted/10 border border-border flex items-center justify-center text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] opacity-30 shadow-inner">
 ...
 <div className="bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-6">
 <div className="text-foreground font-black uppercase tracking-widest leading-none mb-8">Asset Categorization Node</div>
 ...
 <div className="text-lg font-black text-foreground mt-0.5 tracking-tighter">{formatCurrency(c.spent)}</div>
 </div>
 ...
 <div className="bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-6">
 <div className="text-foreground font-black uppercase tracking-widest leading-none mb-6">Top Resource Nodes</div>
 ...
 <div>
 <div className="text-xs font-black text-foreground group-hover:text-muted-foreground transition-all uppercase tracking-tighter ">{s.name}</div>
 ...
 <div className="bg-muted/40 backdrop-blur-xl border border-border shadow-xl rounded-lg p-6">
 <div className="text-foreground font-black uppercase tracking-widest leading-none mb-6">Intelligence Protocol</div>
 ...
 <span className="text-foreground font-black tracking-tighter">{data?.totalOrders || 0}</span>
 </div>
 <div className="rounded-xl border border-border bg-muted/20 p-4 flex items-center justify-between">
 <span className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Active Stream</span>
 <span className="text-foreground font-black tracking-tighter">{data?.activeShipments || 0}</span>
 </div>
 </>
 )}
 </div>
 </div>
 <div className="bg-muted/20 border border-border shadow-xl rounded-lg p-6">
 <div className="text-foreground font-black uppercase tracking-widest leading-none mb-6">Node Operations</div>
 <div className="space-y-3">
