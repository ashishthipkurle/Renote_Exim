        ...
        <div className="max-w-[1800px] mx-auto flex flex-col lg:flex-row items-center gap-6">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <div className="hidden sm:block">
              <h1 className="text-xl font-black text-foreground italic tracking-tighter uppercase leading-none">Procurement</h1>
          ...
          <div className="relative group/search flex-1 max-w-2xl mx-auto w-full">
            ...
            <input
              ...
              className="w-full bg-muted border border-border rounded-2xl py-3.5 pl-14 pr-6 text-sm text-foreground font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all placeholder:text-muted-foreground shadow-inner"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 flex-shrink-0">
            ...
            {activeTab === "browse" && (
              <button
                ...
                className={`p-3 rounded-2xl border transition-all ${showFilters ? "bg-primary text-primary-foreground border-transparent shadow-lg shadow-primary/20" : "bg-muted border border-border text-muted-foreground hover:text-foreground hover:bg-muted/80"}`}
              >
              ...
              <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {wishlist.length === 0 && !loading ? (
                  <div className="py-40 text-center opacity-40">
                    ...
                    <h2 className="text-xl font-black text-foreground uppercase tracking-widest italic">Wishlist Vacant</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-8">
                    {loading ? Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className="h-56 bg-muted/20 border border-border rounded-[2.5rem] animate-pulse" />
                    )) : wishlist.map((item) => (
              ...
              <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {!browseData?.products?.length && !loading ? (
                  <div className="py-20 text-center opacity-40">
                    ...
                    <h2 className="text-xl font-black text-foreground uppercase tracking-widest italic">No Strategic Matches</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8">
                    {loading ? Array.from({ length: 8 }).map((_, i) => (
                      <div key={i} className="h-80 bg-muted/40 border border-border rounded-[2.5rem] animate-pulse" />
                  ...
                  <div className="flex justify-center flex-wrap gap-2 pt-12 border-t border-border mt-12">
                    {Array.from({ length: browseData.pagination.totalPages }, (_, i) => i + 1).map((p) => (
                      <button
                        key={p}
                        onClick={() => setPage(p)}
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xs font-black transition-all ${p === page ? "bg-primary text-primary-foreground border-transparent shadow-xl shadow-primary/10" : "bg-muted text-muted-foreground hover:text-foreground border border-border"}`}
                      >
                        {p}
                      </button>
                    ))}
        ...
        <aside className={`fixed lg:absolute top-0 right-0 h-full w-80 lg:w-96 border-l border-border bg-card z-40 transform transition-transform duration-500 ease-in-out shadow-2xl ${showFilters ? "translate-x-0" : "translate-x-full"}`}>
          <div className="flex flex-col h-full">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-xs text-foreground placeholder:text-muted-foreground/30 outline-none focus:ring-1 focus:ring-primary/20 transition-all font-black italic shadow-inner"
                  />
                  <input
                    type="number"
                    placeholder="Max"
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-muted border border-border rounded-xl py-3.5 pl-11 pr-4 text-xs text-foreground placeholder:text-muted-foreground/30 outline-none focus:ring-1 focus:ring-primary/20 transition-all font-black italic shadow-inner"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="p-8 border-t border-border">
              <button
                onClick={() => setShowFilters(false)}
                className="w-full bg-primary py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-primary-foreground shadow-xl shadow-primary/20 active:scale-95 transition-all"
              >
                Apply Intelligence
              </button>
            </div>
...
function MarketplaceProductCard({ product, onToggleWishlist }: { product: Product; onToggleWishlist: () => void }) {
  return (
    <div className="group bg-muted/20 backdrop-blur-xl border border-border hover:border-primary/20 transition-all rounded-[2.5rem] overflow-hidden shadow-2xl relative">
      ...
      <button
        ...
        className="absolute top-6 right-6 p-3 rounded-2xl bg-background/80 backdrop-blur-md border border-border text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-all active:scale-95 z-10"
      >
...
function WishlistCard({ item, onToggle }: { item: WishlistItem; onToggle: () => void }) {
  return (
    <div className="group bg-muted/20 backdrop-blur-xl border border-border hover:border-primary/20 transition-all rounded-[2.5rem] p-6 relative overflow-hidden shadow-xl">
      ...
      <div className="flex gap-6">
        <div className="size-28 rounded-3xl bg-muted border border-border flex-shrink-0 overflow-hidden relative">
          {item.images?.[0] ? (
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          <div>
            <h3 className="text-sm font-black text-foreground group-hover:text-primary transition-all line-clamp-1 uppercase italic tracking-tighter">{item.name}</h3>
      ...
      <div className="mt-6 pt-6 border-t border-border flex items-center gap-4">
        <Link
          ...
          className="flex-1 bg-muted hover:bg-muted/80 text-foreground border border-border py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-center transition-all"
        >
          Intelligence Report
        </Link>
        <button className="p-3.5 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground border-transparent shadow-xl shadow-primary/20 transition-all active:scale-95 group/cart">
